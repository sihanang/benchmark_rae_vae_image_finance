...
class SNRegularisedEncoder(nn.Module):
    def __init__(self, latent_dims):
        super(SNRegularisedEncoder, self).__init__()
...
class SNRegularisedAutoencoder(nn.Module):
    def __init__(self, latent_dims=64):
        super(SNRegularisedAutoencoder, self).__init__()
        self.encoder = SNRegularisedEncoder(latent_dims)
        self.decoder = SNDecoder(latent_dims)
...
def cars_model_run(config, train_loader = train_dataloader, val_loader = val_dataloader, epochs = 20, lr=0.005, 
              reg_type = 'l2',
    ...
    if reg_type == 'spec_norm':
        autoencoder = SNRegularisedAutoencoder(latent_dims)
    else:
...
rae, train_loss, val_loss= cars_model_run({'beta':beta, 'lamb':lamb}, train_dataloader, val_dataloader, epochs = epochs, lr = lr, 
                                        reg_type = 'spec_norm',
                                        PATH = './', latent_dims = 512, appendum = 'msvae_2layers_enc_v2_512aa_final')
